...
async function fetchLatestRanking(showAlert=false){
  const statusEl=$("rankingSyncStatus");
  try{
    if(statusEl) statusEl.textContent="最新ランキングを確認中…";
    const response=await fetch(`latest_ranking.csv?ts=${Date.now()}`,{cache:"no-store"});
    if(!response.ok) throw new Error(`HTTP ${response.status}`);
    const text=await response.text();
    const rows=parseCSV(text);
    const data=rows.map((r,i)=>({
      date:pick(r,["予測対象日","対象日","日付"]),
      rank:n(pick(r,["AI順位","V5参考順位","V5順位","統合順位","順位"]))||i+1,
      machine:n(pick(r,["現在台番号","台番号","分析用台番号"])),
      score:n(pick(r,["V5参考スコア","V8補正後スコア","排他的期待スコア","V5総合スコア","予測点","期待値点"])),
      decision:pick(r,["AI判定","統合判断","打つ日判定","最終判定","推奨区分"]),
      confidence:pick(r,["信頼度","信頼度星"]),
      reason:pick(r,["見送り理由","判定理由","理由"]),
      dataVersion:pick(r,["データ版"]), generatedAt:pick(r,["生成日時"]), dataHash:pick(r,["データハッシュ"])
    })).filter(x=>x.machine);
    if(!data.length) throw new Error("ランキング行が見つかりません");
    set(K.rank,data);
    renderRank();
    const date=data[0].date||"日付不明";
    if(statusEl) statusEl.textContent=`自動同期済み：${date}（${data.length}台）`;
    if(showAlert) alert(`${date}の最新ランキングを取得しました`);
    return true;
  }catch(err){
    console.warn("latest ranking fetch failed",err);
    if(statusEl) statusEl.textContent="自動取得できませんでした。前回保存分を表示しています。";
    if(showAlert) alert("最新ランキングの取得に失敗しました。GitHubへの同期状況を確認してください。");
    renderRank();
    return false;
  }
}
...
function deleteRecord(recordId){
  const list=get(K.records,[]);
  const target=list.find(x=>x.id===recordId);
  if(!target) return alert("削除対象の記録が見つかりません");

  const label=`${target.date||"日付不明"} ${target.machine||"—"}番台\n収支 ${target.profit>0?"+":""}${yen(target.profit)}`;
  if(!confirm(`この実戦記録を削除しますか？\n\n${label}\n\nこの操作は元に戻せません。`)) return;

  const verify=prompt("誤削除防止のため「削除」と入力してください");
  if(verify!=="削除"){
    if(verify!==null) alert("入力が一致しないため削除を中止しました");
    return;
  }

  const updated=list.filter(x=>x.id!==recordId);
  set(K.records,updated);

  const draft=get(K.draft,null);
  if(draft && draft.id===recordId){
    localStorage.removeItem(K.draft);
    $("playForm").dataset.id=uid();
  }

  renderHistory();
  alert("実戦記録を削除しました。次回CSV書き出しには含まれません。");
}
window.HokutoDeleteRecord=deleteRecord;
  ...
  list.forEach(x=>{
    ...
    el.innerHTML=`
      <div class="history-head">
        <div>
          <strong>${x.date}　${x.machine}番台</strong>
          <div class="history-meta">${x.result}／AI順位 ${x.aiRank||"—"}／当たり${(x.hits||[]).length}回／投資${yen(x.cashInvestment)}</div>
        </div>
        <span class="profit ${x.profit>=0?"positive":"negative"}">${x.profit>0?"+":""}${yen(x.profit)}</span>
      </div>
      <div class="history-actions">
        <button type="button" class="delete-record-button" aria-label="${x.date} ${x.machine}番台の実戦記録を削除">この記録を削除</button>
      </div>`;
    el.querySelector(".delete-record-button").addEventListener("click",()=>deleteRecord(x.id));
    root.appendChild(el);
  });
}
...
async function share(content,name,type){
  ...
  if(navigator.canShare && navigator.canShare({files:[f]})) await navigator.share({files:[f],title:"北斗AI V8.2"});
...
function bind(){
  ...
  $("refreshRanking").addEventListener("click",()=>fetchLatestRanking(true));
  ...
  $("exportBackup").addEventListener("click",()=>share(JSON.stringify({version:VERSION,rank:get(K.rank,[]),records:get(K.records,[]),draft:get(K.draft,null),settings:settings()},null,2),`hokuto_v82_backup_${today()}.json`,"application/json"));
...
document.addEventListener("DOMContentLoaded",()=>{
  try{
    bind(); status(); renderRank(); loadDraft(); renderHistory(); fetchLatestRanking(false);
    if("serviceWorker" in navigator){
      navigator.serviceWorker.register("service-worker-v8.2.js?v=8201").catch(console.warn);
    }
  }catch(err){
    console.error("V8.2 initialization error",err);
    alert("V8.2の初期化でエラーが発生しました。画面を再読み込みしてください。");
  }
});
})();