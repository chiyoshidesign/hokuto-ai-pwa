...
async function share(content,name,type){
  ...
  if(navigator.canShare && navigator.canShare({files:[f]})) await navigator.share({files:[f],title:"北斗AI V7.2"});
...
function bind(){
  ...
  $("exportBackup").addEventListener("click",()=>share(JSON.stringify({version:VERSION,rank:get(K.rank,[]),records:get(K.records,[]),draft:get(K.draft,null),settings:settings()},null,2),`hokuto_v72_backup_${today()}.json`,"application/json"));
...
document.addEventListener("DOMContentLoaded",()=>{
  try{
    ...
    if("serviceWorker" in navigator){
      navigator.serviceWorker.register("service-worker-v7.2.js?v=7201").catch(console.warn);
    }
  }catch(err){
